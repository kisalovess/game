...
document.addEventListener("keydown", e => {
    ...
    if (e.key.toLowerCase() === "q") {
        if (storedShield > 0 && !gameOver && !gameWon && !levelComplete) {
            storedShield--;
    ...
    if (e.key.toLowerCase() === "e") {
        if (storedMulti > 0 && !gameOver && !gameWon && !levelComplete) {
            storedMulti--;
...
function drawPowerUps() {
    for (let p of powerUps) {
        ctx.fillStyle = p.type === "shield" ? "#BAB170" : "#E76F51";
...
function drawPaddle() {
    ctx.fillStyle = "#3A706A";
    ctx.fillRect(paddle.x, paddle.y, paddle.w, paddle.h);

    if (shield) {
        ctx.beginPath();
        ctx.strokeStyle = "#BAB170";
        ctx.lineWidth = 4;
...
function drawBricks() {
    const colors = ["#F6E789", "#F6D675", "#D17341", "#DBB19E", "#4F8286", "#ADC8A6", "#405F47"];
...
function drawScore() {
    ...
    ctx.fillText("Level: " + level, 110, 25);
    ctx.fillText("Shield [Q]: " + storedShield, 220, 25);
    ctx.fillText("Multi [E]: " + storedMulti, 400, 25);

    if (shield) {
        let timeLeft = Math.ceil((shieldEndTime - Date.now()) / 1000);
        ctx.fillText("Shield: " + timeLeft + "s", 570, 25);
    }
}

function drawGameOver() {
    drawOverlay("GAME OVER", "Press RESTART to play again", "white");
}

function drawWinScreen() {
    drawOverlay("YOU WIN!", "Press RESTART to play again", "lime");
}

function drawLevelComplete() {
    drawOverlay("LEVEL COMPLETE", "Press N for next level", "yellow");
}
...
function drawOverlay(title, subtitle, color) {
    ...
    ctx.fillStyle = color;
    ctx.font = "50px Arial";
    ctx.fillText(title, canvas.width / 2 - 190, canvas.height / 2 - 20);

    ctx.fillStyle = "white";
    ctx.font = "24px Arial";
    ctx.fillText(subtitle, canvas.width / 2 - 150, canvas.height / 2 + 30);
}
